...
export const updateCartItemQuantity = createAsyncThunk(
  ...
  async ({ productId, quantity, guestId, userId, size, color }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data || "Update quantity failed");
    }
  }
);
  ...
  async ({ productId, guestId, userId, size, color }, { rejectWithValue }) => {
    try {
      const response = await axios.delete(
        `${import.meta.env.VITE_BACKEND_URL}/api/cart`,
        { params: { productId, guestId, userId, size, color } } // ✅ QUERY PARAMS
      );
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response?.data || "Remove from cart failed");
    }
  }
);
